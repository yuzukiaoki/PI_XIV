...
async def get_artwork(art, session, path, 
        ...
        :param session     => 登入要用的session
        :param path        => 作品要存放的路徑
        :param scrape_type => 爬取類型，預設author
        :param id_begin    => 爬取ID起始，author使用，預設 0
        :param second      => 是否進入第二輪爬取，rank使用，預設False
        :param favorites   => 收藏數量，tag使用，預設None
        獲取Pixiv作品
        """
...
async def check_favorites(session, url, art_id, favorites):
        """
        :param session     => 登入要用的session
        :param url         => 作品Json資料網址
        :param art_id      => 作品ID
        :param favorites   => 指定收藏數
        比較作品是否大於指定收藏數及是否再黑名單
        """
